...
void Scene::InitLights(Shader* _shader)
{
...
void Scene::UpdateLights(Shader* _shader)
{
    ...
    _shader->SetInt("nbrDirectionalLights", dirLights.size());
    for (unsigned int i = 0; i < dirLights.size(); ++i)
    {
        dirLights[i]->SetDirectionalLight(_shader);
    }

    _shader->SetInt("nbrPointLights", pointLights.size());
    for (unsigned int i = 0; i < pointLights.size(); ++i)
    {
        pointLights[i]->SetPointLight(_shader);
    }

    _shader->SetInt("nbrSpotLights", spotLights.size());
    for (unsigned int i = 0; i < spotLights.size(); ++i)
    {
        spotLights[i]->SetSpotLight(_shader);
    }
}
