...
void WorldBuilderWindow::Update()
{
    ...
    UpdateCreateDefaultTerrainPopup(m_isCreateDefaultTerrainPopupActive, m_newDefaultTerrainResolutionX, m_newDefaultTerrainResolutionZ, m_newDefaultTerrainSize);
...
void WorldBuilderWindow::UpdateCreateDefaultTerrainPopup(bool _isPopupOpen, int _xResolution, int _zResolution, Vector2 _size)
{
    ...
    if (ImGui::BeginPopupModal(CREATE_TERRAIN_POPUP_TITLE.c_str(), &_isPopupOpen))
    {
        ...
        {
            WorldBuilder::CreateTerrain(_xResolution, _zResolution, {4.0f, 4.0f});
            _isPopupOpen = false;
